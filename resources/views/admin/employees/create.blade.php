...
<div class="max-w-2xl">
    ...
    <form method="POST" action="{{ route('admin.employees.store') }}" class="bg-white rounded-lg shadow p-6 space-y-4">
        @csrf
        ...
        <div>
            ...
            @if (empty($users))
                <p class="text-xs text-red-500 mt-1">All users are already employees.</p>
            @endif
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" id="base_salary" name="base_salary" value="{{ old('base_salary') }}" required step="0.01" min="0"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    placeholder="0.00">
            </div>
            <div>

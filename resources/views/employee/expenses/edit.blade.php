...
<div class="max-w-2xl">
    ...
    <form method="POST" action="{{ route('employee.expenses.update', $expense->id) }}" class="bg-white rounded-lg shadow p-6 space-y-4">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <select id="category" name="category" required
                    ...
                    <option value="__custom__" {{ old('category', $expense->category) && !in_array(old('category', $expense->category), ['Travel','Food','Office Supplies','Utilities','Software','Training','Other']) ? 'selected' : '' }}>Other (Custom)</option>
                </select>
            </div>
            <div>
                <label for="amount" class="block text-sm font-medium text-gray-700 mb-1">Amount</label>
                <input type="number" id="amount" name="amount" value="{{ old('amount', $expense->amount) }}" required step="0.01" min="0.01"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500">
            </div>
        </div>

        <div id="customCategoryDiv" class="{{ old('category', $expense->category) && !in_array(old('category', $expense->category), ['Travel','Food','Office Supplies','Utilities','Software','Training','Other']) ? '' : 'hidden' }}">
            ...
            <input type="text" id="custom_category" name="custom_category"
                value="{{ old('category', $expense->category) && !in_array(old('category', $expense->category), ['Travel','Food','Office Supplies','Utilities','Software','Training','Other']) ? old('category', $expense->category) : old('custom_category') }}"
                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
        ...
        <div>
            ...
            <input type="date" id="expense_date" name="expense_date" value="{{ old('expense_date', \Carbon\Carbon::parse($expense->expense_date)->format('Y-m-d')) }}" required
                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500">
        </div>
